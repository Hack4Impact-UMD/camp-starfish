...
export async function createGoogleCredentials(uid: string, credentials: Credentials, instance?: Transaction | WriteBatch): Promise<void> {
  await createDoc<Credentials, Credentials>(adminDb.collection(RootLevelCollection.GOOGLE_OAUTH2_TOKENS).doc(uid) as DocumentReference<Credentials, Credentials>, credentials, googleCredentialsFirestoreConverter, instance);
}
