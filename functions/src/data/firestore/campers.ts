...
export async function createCamper(camper: CamperID, instance?: Transaction | WriteBatch): Promise<void> {
  await createDoc<CamperID, Camper>(adminDb.collection(Collection.CAMPERS).doc(String(camper.id)) as DocumentReference<CamperID, Camper>, camper, camperFirestoreConverter, instance);
};
