...
import { SectionSchedule, BundleActivity, SchedulingSectionType } from '@/types/sessionTypes';
...
export function PrefSheet<T extends SchedulingSectionType>({
  ...
  const ageGroups =
    sectionType === 'BUNDLE' ? (['NAV', 'OCP'] as const) : ([undefined] as const);
      ...
      {ageGroups.map((ageGroup) => (
        <Page key={ageGroup ?? 'all'} size="A4" style={styles.page}>
          ...
          <Text style={styles.title}>
            {ageGroup
              ? `${ageGroup === 'NAV' ? 'Navigator' : 'OCP'} ${sectionName} Preference Sheet`
