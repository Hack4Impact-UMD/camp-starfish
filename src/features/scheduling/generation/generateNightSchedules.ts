...
export function useGenerateNightSchedules() {
  return useMutation({
    mutationFn: async (req: UseGenerateNightSchedulesRequest, { client }) => {
      ...
      const adminIds = (await client.ensureInfiniteQueryData(getUseAttendeeListOptions(sessionId, { where: [{ fieldPath: "role", operation: "in", value: ["ADMIN"] }] }))).pages.flatMap(page => page.docs.map(attendee => attendee.attendeeId));
...
function generateNightSchedulesForBunk(bunk: Bunk, daysOffSchedule: DaysOffSchedule, session: Session): { [date: string]: Record<NightSchedulePosition, number[]>; } {
  ...
  for (let currDate = session.startDate.clone(); currDate.isBefore(session.endDate, "day"); currDate = currDate.clone().add(1, 'day')) {
    ...
    const isCurrDateADayOff = daysOffSchedule.daysOffInSession.some(dayOff => dayOff.isSame(currDate, 'day'));
    const isTomorrowADayOff = daysOffSchedule.daysOffInSession.some(dayOff => dayOff.isSame(currDate.clone().add(1, 'day')));
    for (const counselorId of getObjectKeysAsNumbers(daysOffSchedule.daysOffByCounselorId)) {
      ...
      if (isCurrDateADayOff && daysOffSchedule.daysOffByCounselorId[counselorId].some(day => day.isSame(currDate, 'day'))) {
        counselorsWithDayOff.push(counselorId);
      }
      if (isTomorrowADayOff && daysOffSchedule.daysOffByCounselorId[counselorId].some(day => day.isSame(currDate.clone().add(1, 'day')))) {
