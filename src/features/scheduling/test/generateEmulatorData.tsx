import { CamperAttendeeID, StaffAttendeeID, AdminAttendeeID, AgeGroup, SectionSchedule, BundleActivityWithAssignments, 
 SectionPreferences, BunkID, BunkJamboreeActivityWithAssignments,NonBunkJamboreeActivityWithAssignments, Freeplay, 
  ...
  ProgramAreaID,
  SchedulingSectionID,
  FreeplayID} from "@/types/sessionTypes";
...
// TO_DO: Add program counselors for WF (one per each bunk)
...
function generateFreeplayBlockSchedule(posts: PostID[], id: string, session: string) : FreeplayID {

  const schedule: FreeplayID = { posts: {}, buddies: {}, id: id, sessionId: session };
...
// export function generateBunkJamboreeSchedule(numBlocks: number, campers: CamperAttendeeID[], staff: StaffAttendeeID[], admins: AdminAttendeeID[], section: SectionID) {
...
//   const preferences: SectionPreferences = generatePrefs(bunks, schedule, "BUNK-JAMBO");
...
// export function generateFreeplaySchedule(campers: CamperAttendeeID[], staff: StaffAttendeeID[], admins: AdminAttendeeID[], id: string, session: string)
// {

//   const posts: PostID[] = generatePosts(10);
//   const schedule: FreeplayID = generateFreeplayBlockSchedule(posts, id, session);
...
export function generateSession() {
  ...
  // const bunkJamboreeScheduler = generateBunkJamboreeSchedule(5, campers, staff, admins, sections[2]);
  //const nonBunkJamboreeScheduler = generateNonBunkJamboreeSchedule(5, campers, staff, admins, sections[4]);
  //const freeplayScheduler1 = generateFreeplaySchedule(campers, staff, admins, sections[1].startDate, sections[1].sessionId);

  // const freeplay2Date = moment(sections[1].startDate).add(1, "days").toISOString();
  // const freeplayScheduler2 = generateFreeplaySchedule(campers, staff, admins, freeplay2Date, sections[1].sessionId);
  // freeplayScheduler2.withOtherFreeplays([freeplayScheduler1.schedule]);

  // const freeplay3Date = moment(sections[1].startDate).add(2, "days").toISOString();
  // const freeplayScheduler3 = generateFreeplaySchedule(campers, staff, admins, freeplay3Date, sections[1].sessionId);
  // freeplayScheduler3.withOtherFreeplays([freeplayScheduler1.schedule, freeplayScheduler2.schedule]);


  //const allFreeplays = [freeplayScheduler1];
  return {
    // bundleScheduler1,
    // bundleScheduler2,
    // bundleScheduler3,
