...
import { getObjectEntriesWithNumberKeys, getObjectKeysAsNumbers } from "@/utils/stringUtils";
...
export function AddAttendeesModal(props: AddAttendeesModalProps) {
  ...
  const potentialSessionAttendees = useMemo(() => {
    ...
    return Object.fromEntries(getObjectEntriesWithNumberKeys(userDirectoryQuery.data || {}).filter(([userId, user]) => isAttendeeRole(user.role) && !sessionQuery.data.attendeeIds.includes(userId)));
