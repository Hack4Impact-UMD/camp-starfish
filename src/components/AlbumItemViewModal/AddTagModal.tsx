...
import { Button, Loader, MultiSelect, Pill, Text, Title } from "@mantine/core";
import { MdError, MdGroups, MdSearch } from "react-icons/md";
...
function AddTagModalContent(props: AddTagModalProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 px-2 pb-2">
      ...
      <MultiSelect
        className="w-full"
        placeholder={selectedIds.length === 0 ? "Search tags..." : ""}
        ...
        renderOption={(optionInput) =>
          userDirectory ? (
            <Text className="text-sm">
              {getFullName(userDirectory[Number(optionInput.option.value)].name)}
            </Text>
          ) : null
        }
        ...
        <Button
          ...
          onClick={() =>
            createTagMutation.mutate(
              { albumId, albumItemId, tagIds: selectedIds.map(Number) },
              {
                onSuccess: () => setSucceeded(true),
                onError: () =>
                  notifications.error("Failed to add tag. Please try again."),
              },
            )
          }
        >
...
export default function openAddTagModal(albumId: string, albumItemId: string) {
  modals.open({
    withCloseButton: false,
    children: (
      <AddTagModalContent albumId={albumId} albumItemId={albumItemId} />
    ),
  });
}
