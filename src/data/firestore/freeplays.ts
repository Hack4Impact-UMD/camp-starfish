...
export async function createFreeplay(sessionId: string, freeplay: Freeplay, instance?: Transaction | WriteBatch): Promise<void> {
  await createDoc<Freeplay, FreeplayDoc>(doc(db, Collection.SESSIONS, sessionId, SessionsSubcollection.FREEPLAYS, freeplay.date) as DocumentReference<Freeplay, FreeplayDoc>, freeplay, freeplayFirestoreConverter, instance);
};
