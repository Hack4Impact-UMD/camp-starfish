...
export async function setImage(albumId: string, imageId: string, image: ImageMetadata, instance?: Transaction | WriteBatch): Promise<void> {
  await setDoc(doc(db, Collection.ALBUMS, albumId, AlbumsSubcollection.IMAGE_METADATAS, imageId) as DocumentReference<ImageMetadataID, ImageMetadata>, { id: imageId, albumId, ...image }, imageFirestoreConverter, instance);
}
